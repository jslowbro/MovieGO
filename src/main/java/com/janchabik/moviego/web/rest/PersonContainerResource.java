...
public class PersonContainerResource {
    ...
    public List<PersonContainer> getAllPersonContainers(@RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        log.debug("REST request to get all PersonContainers");
        return personContainerRepository.findAllWithEagerRelationships();
    }
    ...
    public ResponseEntity<PersonContainer> getPersonContainer(@PathVariable Long id) {
        ...
        Optional<PersonContainer> personContainer = personContainerRepository.findOneWithEagerRelationships(id);
