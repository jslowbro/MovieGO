...
public class FilmResourceIT {
    ...
    public void getAllFilmsWithEagerRelationshipsIsEnabled() throws Exception {
        when(filmServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restFilmMockMvc.perform(get("/api/films?eagerload=true"))
            .andExpect(status().isOk());
    ...
    @SuppressWarnings({"unchecked"})
    public void getAllFilmsWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(filmServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));
