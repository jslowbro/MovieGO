...
public class PersonContainerResourceIT {
    ...
    @Mock
    private PersonContainerRepository personContainerRepositoryMock;
    ...
    public void getAllPersonContainersWithEagerRelationshipsIsEnabled() throws Exception {
        PersonContainerResource personContainerResource = new PersonContainerResource(personContainerRepositoryMock);
        when(personContainerRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restPersonContainerMockMvc.perform(get("/api/person-containers?eagerload=true"))
            .andExpect(status().isOk());

        verify(personContainerRepositoryMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({"unchecked"})
    public void getAllPersonContainersWithEagerRelationshipsIsNotEnabled() throws Exception {
        PersonContainerResource personContainerResource = new PersonContainerResource(personContainerRepositoryMock);
        when(personContainerRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));
