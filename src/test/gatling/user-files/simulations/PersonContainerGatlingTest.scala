...
/**
 * Performance test for the PersonContainer entity.
 */
class PersonContainerGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the PersonContainer entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all personContainers")
            .get("/api/person-containers")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new personContainer")
            .post("/api/person-containers")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "role":"ACTOR"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_personContainer_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created personContainer")
                .get("${new_personContainer_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created personContainer")
            .delete("${new_personContainer_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
